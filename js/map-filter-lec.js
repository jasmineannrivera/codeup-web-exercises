...
$(document).ready(function() {
    ...
    //  3. Use ES6
        ...
        let filterES6 = mbpModels.filter(mbp => getYear(mbp.year) >= 2011);

        $("#filterEs6").html(createList(filterES6.map(mbp => mbp.desc + "<strong>(" + mbp.year + ")</strong> ")));
    ...
    let forEachTotal = 0;
    mbpModels.forEach(function (mbp) {
        forEachTotal += mbp.price;
    });
    $("#reduceForEach").html(commaThousands(forEachTotal));
    /********************************* 2 *********************************/
    //  2. Use reduce()


   let reduceTotal = mbpModels.reduce(function (total, mbp) {
        return total + mbp.price;
    }, 0);
    $("#reduce").html(commaThousands(reduceTotal));
    /********************************* 3 *********************************/
    //  3. Use ES6

    let ES6total = mbpModels.reduce((total, mbp) => total + mbp.price, 0);

    $("#reduceEs6").html(commaThousands(ES6total));
